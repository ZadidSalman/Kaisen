...
export async function GET() {
  ...
  await ThemeCache.updateMany({}, { isPopular: false, popularRank: null, featuredAt: null })
  ...
  for (const pair of pairs) {
    ...
    // 1. Exact or starts-with regex on anime title, combined with word regex on song title
    const songTerms = pair.song.split(/\s+/).slice(0, 2).join(' ') // First two words of song
    const regexAnime = new RegExp(cleanAnime.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')
    const regexSongLoose = new RegExp(songTerms.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')

    let candidates = await ThemeCache.find({
      $or: [
    ...
    // Try to match song inside candidates loosely
    for (const c of candidates) {
      if (c.songTitle.toLowerCase().includes(pair.song.toLowerCase())) {
        themeMatch = c; break;
      }
      if (regexSongLoose.test(c.songTitle)) {
        themeMatch = c; break;
      }
      // Extremely loose match, e.g., 'Cruel Angel' in 'Zankoku na Tenshi no Thesis' is bad, but
      // 'Tank!' in 'Tank!' is good. Let's do a strict inclusion on the first word minimum.
    }

    if (!themeMatch) {
       // Manual overrides for known tough ones based on AnimeThemes DB format
       ...
       const overrideSong = overrides[pair.song] || overrides[pair.song.substring(0, 15)];
       if (overrideSong) {
         themeMatch = candidates.find(c => c.songTitle.toLowerCase().includes(overrideSong.toLowerCase()))
       }
    }

    // 3. Fallback to raw text search if candidates failed
    if (!themeMatch) {
      const textMatches = await ThemeCache.find({
        $text: { $search: `"${pair.song}" "${cleanAnime}"` }
      }).limit(1).lean()
      if (textMatches.length > 0) themeMatch = textMatches[0]
    }
