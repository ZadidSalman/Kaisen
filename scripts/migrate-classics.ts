...
async function migrateClassics() {
  const uri = process.env.MONGODB_URI || '';
  if (!uri) throw new Error('MONGODB_URI not set');

  const testConn = await mongoose.createConnection(uri, { dbName: 'test' }).asPromise();
  const kaiConn = await mongoose.createConnection(uri, { dbName: 'kaikansen' }).asPromise();

  const ThemeSchema = new mongoose.Schema({}, { strict: false });
  const TestTheme = testConn.model('ThemeCache', ThemeSchema);
  const KaiTheme = kaiConn.model('ThemeCache', ThemeSchema);
  ...
  console.log(`Migrating missing classics from 'test' to 'kaikansen' for ${pairs.length} songs...`);
  ...
  for (const pair of pairs) {
    ...
    // 1. Check if it already exists in kaikansen
    const exists = await KaiTheme.findOne({
      $or: [
    ...
    if (exists) {
      console.log(`[EXIST] ${pair.song} already in kaikansen`);
      await KaiTheme.findByIdAndUpdate(exists._id, { isPopular: true, popularRank: pair.rank, featuredAt: new Date() });
      continue;
    }

    // 2. Not in kaikansen, find in test
    console.log(`[SEARCH] Searching for ${pair.song} in 'test' DB...`);
    
    // Fuzzy search in 'test'
    // We try many variations because 'test' has the official names
    const sourceTheme = await TestTheme.findOne({
      $or: [
        { animeTitle: new RegExp(cleanAnime.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i') },
        { animeTitleEnglish: new RegExp(cleanAnime.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i') },
      ...
      $or: [
        { songTitle: new RegExp(pair.song.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i') },
        // Try a very loose match for the song too if it's famous
        { songTitle: new RegExp(pair.song.substring(0, 5).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i') }
      ]
    }).lean();

    if (sourceTheme) {
      console.log(`[MIGRATE] Found ${pair.song} in 'test'! Copying to 'kaikansen'...`);
      const { _id, ...themeData } = sourceTheme;
      await KaiTheme.findOneAndUpdate({ slug: themeData.slug }, {
        ...themeData,
        isPopular: true,
      ...
      console.log(`[MISS] ${pair.song} not found in 'test' either.`);
    }
  }

  console.log(`Finished! Migrated ${migratedCount} themes.`);
  process.exit(0);
}

migrateClassics().catch(console.error);
